            ...
            <form action="">
                <div>
                    ...
                    <div class="mt-1 grid  grid-cols-3">
                        ...
                        <div>
                            @if(request()->get('branch_') != null && request()->get('branch_') != 'الكل')
                        ...
                        <tr  class="status_{!!$shipment->Status_!!}_color"   >
                            ...
                            <td class="whitespace-nowrap " >{{$shipment->code_}}</td>
                                    <td class="whitespace-nowrap " ><input type="checkbox" class="check_count" data-cost='{{$shipment->shipment_coast_}}'
                                        data-t7wel='{{$shipment->t7weel_cost}}' data-net='{{$shipment->shipment_coast_}}' data-code='{{$shipment->code_}}' data-status='{{$shipment->Status_}}' data-arba7='{{$shipment->arba7}}'></td>
                        </tr>
                        @endforeach
        ...
        <script type="text/javascript">
            ...
            $('#operation_print').on('click',function(){
                $.ajax({
                            ...
                            success: function(result){
                                window.open("{{route('opretation-print')}}"+'?report='+result.id+'&brach_filter='+branch_data+'&type=fro3&title='+'شحنات  مسددة للفرع');
            ...
            function infinteLoadMore(page) {
                $.ajax({
                    url: "{{route('accounting.notmosadad')}}"+ "?lodaMore=1&page=" + page+'&'+window.location.search.substr(1),
                    ...
                    $.each(response.data,function(key,value){
                        ...
                        $('#dataTable   tr:last').after(`<tr  class='status_`+value.Status_+`_color'>
                            ...
                            <td class="whitespace-nowrap " ><input type="checkbox" class="check_count" data-cost='`+value.shipment_coast_+`'
                                        data-t7wel='`+value.t7weel_cost+`' data-net='`+value.shipment_coast_+
                                        `' data-code='`+value.code_+`' data-status='`+value.Status_+`' data-arba7='`+value.arba7+`'></td>
                                            </tr>`
                            );
